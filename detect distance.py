...
while True:
    ...
    if all(0 <= command <= required_length+required_length for command in ride_commands):
        rows = 'both'
        cv2.putText(depth_image, "both", (440, 20 ), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 255), 2)
    elif all(0 <= command <= required_length+required_length for command in ride_commands[1::2]):
        rows = 'right'
        cv2.putText(depth_image, "right", (440, 20 ), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 255), 2)
    elif all(0 <= command <= required_length+required_length for command in ride_commands[::2]):
        rows = 'left'
