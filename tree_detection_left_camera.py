...
num, tree_pixel, prev_tree_pixel, center_pixel = [[0, 0] for _ in range(4)]
dist = [112,112]
box = [[[],[],[],[]],[[],[],[],[]]]
old_points = [np.array([[]]),np.array([[]])]
# Создаём первый фрейм для настройки функций
# _, frame = [cap[i].read() for i in range(2)]
# print(frame)
# frame = [cv2.resize(frame[i], (224, 224)) for i in range(2)]
# old_gray = [cv2.cvtColor(frame[i], cv2.COLOR_BGR2GRAY) for i in range(2)]
...
def select_point(count_dist, idx):
...
while True:
    ...
    for i in range(2):
        ...
        if coord[i][0] <= 1 or coord[i][0] >= 224 or coord[i][1] <= 1 or coord[i][1] >= 224:
            print(x[i])
            select_point(abs(224 - coord[i][0]), i)
